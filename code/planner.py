...
def make_full_payment_plan(request_date, amount):
    return [{"date": pd.to_datetime(request_date).date(), "amount": Decimal(str(amount)), "payment_id": "request_payment_1"}]
...
def make_installment_plan(option):
    ...
    return [{"date": (first_date + pd.Timedelta(days=frequency * i)).date(), "amount": amount, "payment_id": f"request_payment_{i + 1}"} for i in range(number_of_payments)]

def plan_end_date(plan):
    return max(payment["date"] for payment in plan)

def plan_total(plan):
    return sum((Decimal(str(payment["amount"])) for payment in plan), Decimal("0"))
...
def candidate_plans(context, state, base_cash_events):
    ...
    desired_date = pd.to_datetime(request["desired_completion_date"]).date()
    amount = Decimal(str(request["requested_amount"]))
    candidates = []

    if "full_payment" in state.payment_methods:
        plan = make_full_payment_plan(request_date, amount)
        candidates.append({
            "method": "full_payment",
            "payment_plan": plan,
    ...
    if "installments" in state.payment_methods:
        for _, option in options.iterrows():
            if str(option["payment_method"]).lower() != "installments":
                continue
            if not option_is_eligible(option, state.max_installment_months):
                continue
            required = ["first_payment_date", "payment_frequency_days", "number_of_payments", "payment_amount"]
...
def validate_candidates(candidates, state, request_date, base_cash_events):
    validated = []
    ...
    for candidate in candidates:
        ...
        if result["safe"]:
            ...
            valid["minimum_balance_seen"] = result["minimum_balance_seen"]
            valid["ending_balance"] = result["ending_balance"]
        ...
        for spending_result in spending_results:
            ...
            valid["spending_changes_needed"] = format_spending_changes(changes)
            valid["minimum_balance_seen"] = spending_result["minimum_balance_seen"]
            valid["ending_balance"] = spending_result["ending_balance"]
            validated.append(valid)
